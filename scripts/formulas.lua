...
function calculate_stats_ingo(reactor,state_running_time)
	...
	local fuel_cell = reactor.displayer.burner.currently_burning
	...
	elseif reactor.state == 1 then --start
		...
		if temperature <= 500 then
			...
			--power=math.min(((39/settings.global["reactor-starting-duration"].value)*state_running_time)+1,40) -- running time: 0s=1MW, start_duration(default:30s)=40MW 
			--power_breeder=math.min(((39/settings.global["reactor-starting-duration"].value)*state_running_time)+1,40) -- running time: 0s=1MW, start_duration(default:30s)=40MW
		else
		...
		power=math.min((((power_target-1)/settings.global["reactor-starting-duration"].value)*state_running_time)+1,power_target) -- running time: 0s=1MW, start_duration(default:30s)= output of a running reactor at this temp 
		power_breeder=math.min((((power_breeder_target-1)/settings.global["reactor-starting-duration"].value)*state_running_time)+1,power_breeder_target) -- running time: 0s=1MW, start_duration(default:30s)= output of a running breeder at this temp			
		efficiency=100
		BonusCellAmount=0
		
	elseif reactor.state == 3 then --scram
		
		power=(((reactor.power_output_last_tick-1)/settings.global["reactor-scram-duration"].value)*(settings.global["reactor-scram-duration"].value-state_running_time))+1 --running time: scram_duration(default:180s)=power_output_last_tick, 0s=1MW
		power_breeder=(((reactor.power_output_last_tick-1)/settings.global["reactor-scram-duration"].value)*(settings.global["reactor-scram-duration"].value-state_running_time))+1 --running time: scram_duration(default:180s)=power_output_last_tick, 0s=1MW
		efficiency=200
		BonusCellAmount=0
	...
	if reactor.displayer.burner.currently_burning == nil then
		--nerf nothing
	elseif fuel_cell.name ==  "mox-fuel-cell" and reactor.displayer.burner.currently_burning.fuel_value == 8500000000 then --mox
		--SigmaOne's Mods: Nuclear fuel (same stats as uranium power)
		bonus_cells_multiplier = 0.825
		
	elseif fuel_cell.name == "plutonium-fuel-cell" then
		--Plutonium Energy
	...
	elseif fuel_cell.name == "MOX-fuel" then
	...
	-- return values	
	if reactor.displayer.name == REACTOR_ENTITY_NAME then	
	...
	if reactor.displayer.name == BREEDER_ENTITY_NAME then 
...
function calculate_stats_ownly(reactor,running_time)
	local fuel_cell = reactor.displayer.burner.currently_burning
	...
	reactor.signals.parameters["neighbour-bonus"].count = reactor.neighbours
	if fuel_cell == nil then
		efficiency=0
		output=0
		
	--SigmaOne's Mods: Nuclear fuel (same stats as uranium power)
	elseif fuel_cell_name ==  "mox-fuel-cell" and reactor.displayer.burner.currently_burning.fuel_value == 8500000000 then --mox
	...
	--Plutonium Energy
	elseif fuel_cell_name == "plutonium-fuel-cell" then --plutonium
	...
	if reactor.state  == 1 then
		output = math.floor(output * ((running_time)/settings.global["reactor-starting-duration"].value)^2)
		bonus_cells = bonus_cells * ((running_time)/settings.global["reactor-starting-duration"].value)^2
	end
	if reactor.state  == 3 then
		output = math.floor(reactor.power_output_last_tick * ((settings.global["reactor-scram-duration"].value - (running_time)/3.5)/settings.global["reactor-scram-duration"].value)^11+0.45)
		bonus_cells = bonus_cells * ((settings.global["reactor-scram-duration"].value - (running_time)/3.5)/settings.global["reactor-scram-duration"].value)^11
	end	
	--game.print(output)
