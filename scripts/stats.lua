...
function stats_tick(event)
if event.tick% (TICKS_PER_UPDATE*2) == 0 then
...
if event.tick % TICKS_PER_UPDATE == 0 then
	for key, gui in pairs (global.guis) do
		...
		if reactor and reactor.displayer.valid and global.stats[reactor_key] then
			...
			if gui.progress then
				if reactor.displayer.get_fuel_inventory().is_empty() then
					gui.progress.cells.number = nil
					gui.progress.cells.sprite = "rr-transparent-sprite"
				else
					gui.progress.cells.number = reactor.displayer.get_fuel_inventory()[1].count
					gui.progress.cells.sprite = "item/"..reactor.displayer.get_fuel_inventory()[1].name
				end
				
				if reactor.displayer.get_burnt_result_inventory().is_empty() then
					...
					local inv1 = reactor.displayer.get_burnt_result_inventory()[1]
					...
					if gui.progress.used_cells2 then
						local inv2 = reactor.displayer.get_burnt_result_inventory()[2]
				...
				if reactor.displayer.burner.currently_burning == nil then
					...
					local burnt_result = reactor.displayer.burner.currently_burning.burnt_result.name
					if burnt_result == "apm_fuel_cell_mox_used" then
						burnt_result = "apm_nuclear_breeder_uranium_inventory_enriched"
					end
					--gui.progress.cells.sprite = "item/"..reactor.displayer.burner.currently_burning.name
					--gui.progress.used_cells.sprite = "item/"..burnt_result.name
					gui.progress.bars.progress.value = reactor.displayer.burner.remaining_burning_fuel / reactor.displayer.burner.currently_burning.fuel_value 
			...
			global.stats[reactor_key] = nil
			gui.destroy()
			global.guis[reactor_key.."-"..playerid] = nil
...
--process graphs
if event.tick % math.floor((TICKS_PER_UPDATE*2) / global.gui_count) ==0 then
	if not global.guis[global.worker_key] then global.worker_key = nil end
	global.worker_key = next(global.guis, global.worker_key)
	if global.worker_key == nil then
		global.worker_key = next(global.guis, global.worker_key)
	end
	local graphs_processed=0
	while global.worker_key and graphs_processed<1 do
		local gui = global.guis[global.worker_key]
		if gui.graph then
			local reactor_key,playerid = splitty(global.worker_key,"-")
			if not global.reactors[reactor_key] or not global.stats[reactor_key] then
				global.stats[reactor_key] = nil
				gui.destroy()
				global.guis[reactor_key.."-"..playerid] = nil
				global.gui_count = global.gui_count - 1
			...
			graphs_processed=graphs_processed+1
		else				
			global.worker_key = next(global.guis, global.worker_key)
		end
	end	
...
script.on_event(defines.events.on_gui_opened, function(event)
  ...
  if entity ~= nil and (entity.name == "realistic-reactor-interface" or entity.name == "realistic-breeder-interface") then
	local reactor_key
	for key, reactor in pairs(global.reactors) do
		if reactor.interface == player.opened then
			reactor_key = key
		end
	end
    player.opened = nil
	if reactor_key then
		if player.gui.left["rr_gui_"..reactor_key] then
			player.gui.left["rr_gui_"..reactor_key].destroy()
			global.guis[reactor_key.."-"..event.player_index]=nil
			global.gui_count = global.gui_count -1
		else
...
script.on_event(defines.events.on_gui_click, function(event)
	...
	local reactor_key = tonumber(string.sub(element.parent.parent.name,8))
	...
	elseif element.name == "status" then
		...
		if reactor.displayer.get_fuel_inventory().is_empty() and reactor.displayer.burner.remaining_burning_fuel == 0 then
			game.players[event.player_index].print("[RealisticReactors] Reactor can't start, because it has no fuel cell.")
		elseif reactor.power.energy < (POWER_USAGE_STARTING * TICKS_PER_UPDATE * 4 / 60* settings.global["rr_energy_consumption_multiplier"].value) then
			game.players[event.player_index].print("[RealisticReactors] Reactor can't start, because it has not enough electricity.")
		elseif signal_scram then
			game.players[event.player_index].print("[RealisticReactors] Reactor can't start, because there's a scram control signal on its circuit interface.")
		else
			if reactor.state == 0 then
				state_changed = true
				change_reactor_state(1, reactor, event)
				update_reactor_states(reactor_key, event)
			elseif reactor.state == 1 or reactor.state == 2 then
				state_changed = true
				change_reactor_state(3, reactor, event)
				update_reactor_states(reactor_key, event)
			else
				game.players[event.player_index].print("[RealisticReactors] Reactor is already scramed.")
			end
		end
...
function create_gui(player, reactor_key)
	local reactor = global.reactors[reactor_key]
...
function create_signals (gui,reactor_key)
	...
	--gui.signals.style.scaleable = false
		...
		if global.reactors[reactor_key].displayer.name=="realistic-reactor-breeder" then
		...
		gui.signals.add{type = "sprite-button", name = "status", sprite="virtual-signal/signal-state-stopped", direction = "vertical", style = "icon_button"}
		gui.signals.status.number=0
		...
		gui.signals.status.ignored_by_interaction =false

end

function create_progress (gui,reactor_key)
	local cols = 3
	if #global.reactors[reactor_key].displayer.get_burnt_result_inventory() > 1 then
		cols = 4
	end
